...
setwd("C:/Users/victm/Dropbox/CogSci 4/Comp modeling/Eye Tracking/portfolio 1/Portfolio-3-Autism")
...
mu.ASD <- link( m7.5 , data=data.frame(ASD=1,NonVerbalIQ=IQ.seq) )
mu.ASD.mean <- apply( mu.ASD , 2 , mean )
mu.ASD.PI <- apply( mu.ASD , 2 , PI , prob=0.97 )
mu.TD <- link( m7.5 , data=data.frame(ASD=0,NonVerbalIQ=IQ.seq) )
mu.TD.mean <- apply( mu.TD , 2 , mean )
mu.TD.PI <- apply( mu.TD , 2 , PI , prob=0.97 )

# plot African nations with regression 7.11
d.A1 <- d4[d4$ASD==1,]
...
#is this the same?
mvariation <- rethinking::map(
  alist(
    ADOS ~ dnorm(mu, sigma),
    mu <- intercept + gamma1*NonVerbalIQ + bgroup1*ASD + 
          gamma2*VerbalIQ + bgroup2*ASD + gamma3*SocialIQ + bgroup3*ASD,
    gamma1 <- bnon + binteraction1*ASD, #nonverbal
    gamma2 <- bverb + binteraction2*ASD, #verbal
    gamma3 <- bsoc + binteraction3*ASD, #social
    intercept ~ dnorm(2.15, 1),
